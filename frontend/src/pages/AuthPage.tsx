...
export const AuthPage = () => {
  ...
  const handleAuth = async (type: 'login' | 'register') => {
    ...
    try {
      ...
      if (type === 'login' && res.token) {
        login(res.token, email);
        navigate('/');
      } else if (type === 'register' && res.userId) {
        const loginRes = await authService.login(email, password);
        if (loginRes.token) {
          login(loginRes.token, email);
          navigate('/');
        }
      } else {
        setError(res.error || `Błąd ${type === 'login' ? 'logowania' : 'rejestracji'}`);
      }
    } catch (err) {
        ...
        <Tabs defaultValue="login" className="flex flex-col w-full">
          ...
          <CardContent className="flex flex-col px-8 pb-10">
            ...
            <TabsList className="grid w-full grid-cols-2 bg-zinc-900/50 rounded-full p-1 mb-8 border border-zinc-800">
              <TabsTrigger value="login" className="rounded-full py-2 data-[state=active]:bg-sky-500 data-[state=active]:text-white transition-all">
                Logowanie
              </TabsTrigger>
              <TabsTrigger value="register" className="rounded-full py-2 data-[state=active]:bg-sky-500 data-[state=active]:text-white transition-all">
                Rejestracja
              </TabsTrigger>
            </TabsList>
