...
export const ProfilePage = () => {
  ...
  const validate = () => {
    if (editingEmail) {
      if (!email) { setError('Email jest wymagany'); return false; }
      const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
      if (!emailRegex.test(email)) { setError('Podaj prawidłowy adres email'); return false; }
    }
    if (editingUsername && username) {
      if (username.length < 3) { setError('Minimum 3 znaki'); return false; }
      if (username.length > 20) { setError('Maksymalnie 20 znaków'); return false; }
      if (!/^[a-zA-Z0-9_]+$/.test(username)) { setError('Tylko litery, cyfry i _'); return false; }
    }
    return true;
  };
  ...
  if (!localStorage.getItem('token')) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] p-4">
        ...
        <Button variant="outline" onClick={() => navigate('/auth')} className="cursor-pointer">Przejdź do logowania</Button>
      </div>
    );
  }
    ...
    <div className="flex flex-col items-center justify-center min-h-[80vh] p-4">
      <Card className="w-full max-w-lg bg-black border border-zinc-800 rounded-3xl">
        ...
        <CardContent className="space-y-4 pb-8">
          ...
          {success && <div className="text-sm text-emerald-300 p-2 bg-emerald-500/10 rounded-md">{success}</div>}
          ...
          <div className="space-y-2">
            <label className="text-xs uppercase text-zinc-400">
              Nazwa użytkownika {editingUsername && <span className="text-sky-400 normal-case">(edytujesz)</span>}
            </label>
            <Input
              value={username}
          ...
          <div className="flex gap-2 mt-2">
            ...
            <Button variant="outline" onClick={logout} className="w-1/2 cursor-pointer">Wyloguj</Button>
          </div>
        </CardContent>
      </Card>
    </div>
